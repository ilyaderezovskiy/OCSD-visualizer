...
export default function LoadSankeyData(load, filterData, minMax, initSlider, minMaxNode, initSliderNode, nodes, energy2, vertices2) {
  ...
  var data = results.data.map(function(item){return {'id': item.item, 'source': item.source, 'target': item.target, 'cycle_1': item.cycle_1};});
  ...
  Array.prototype.forEach.call(uniqueData, element => {
    ...
    if (typeof element.source !== 'undefined' && typeof element.target !== 'undefined') {
      el.push({'source': element.source, 'target': element.target, 'cycle_1': cycles[element.source], 'items': items[element.source], 'value': value, 'valueLabel': value })
    }
  });
  ...
  // el = el.filter(item => !(item.target.includes('null')));
  // el = el.filter(item => !(item.source.includes('null')));
