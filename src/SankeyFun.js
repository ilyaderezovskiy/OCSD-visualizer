...
export default function SankeyFun (
  ...
  const lightYellowColor = d3.rgb(255, 215, 0).copy({opacity: 0.3});
  const lightRedColor = d3.rgb(255,204,229).copy({opacity: 1}); //d3.rgb(255, 99, 71).copy({opacity: 0.4});
  const lightGreyColor = d3.color("grey").copy({opacity: 0.1});
  ...
  const node = svg
    ...
    .attr("x", (d) => d.x0)
    .attr("y", (d) => d.id.includes('pas') ? d.y0 + 1 : d.y0)
    .attr("height", (d) => d.id.includes('pas') ? Math.max(4, d.y1 - d.y0 - 2) : Math.max(4, d.y1 - d.y0))
...
//Добавление бар-чартов
  ...
  function drawGroupBorders() {
    // Фильтруем узлы, исключая 'pas' и 'col'
    const filteredNodes = nodes.filter(node => 
      !node.id.includes('pas') && !node.id.includes('col')
    );
  
    // Группируем по глубине (колонкам)
    const groups = d3.group(filteredNodes, d => d.depth);
    
    groups.forEach((groupNodes, depth) => {
      if (groupNodes.length === 0) return;
      
      // Границы группы
      const x0 = d3.min(groupNodes, d => d.x0) - 3;  // Отступ
      const x1 = d3.max(groupNodes, d => d.x1) + 3;
      const y0 = d3.min(groupNodes, d => d.y0) - 3;
      const y1 = d3.max(groupNodes, d => d.y1) + 3;
      ...
      svg.append("rect")
        ...
        .attr("width", x1 - x0)
        .attr("height", y1 - y0)
        // .attr("rx", 12)  // Скругление углов
        // .attr("ry", 12)
        .attr("fill", "none")
        .attr("stroke", "#555")  // Цвет обводки
        .attr("stroke-width", 2)
        //.attr("stroke-dasharray", "4,4")
        .style("pointer-events", "none")
        .raise();  // Выводим на верхний уровень
    });
  }
